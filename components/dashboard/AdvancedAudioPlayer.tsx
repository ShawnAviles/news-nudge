...
export function AdvancedAudioPlayer({ 
  ...
  const updateCurrentSegment = useCallback((time: number) => {
    ...
    if (segment && segment.id !== currentSegmentId) {
      ...
      if (transcriptRef.current) {
        const segmentElement = transcriptRef.current.querySelector(`[data-segment-id="${segment.id}"]`);
        if (segmentElement) {
  ...
  const playNext = () => {
    ...
    const currentIndex = recentTranscripts.findIndex(t => t.id === currentTranscript.id);
  ...
  const playPrevious = () => {
    ...
    const currentIndex = recentTranscripts.findIndex(t => t.id === currentTranscript.id);
    const prevIndex = currentIndex === 0 ? recentTranscripts.length - 1 : currentIndex - 1;
    ...
    return (
      <Card className={className}>
        <CardContent className="p-6 text-center">
          <p className="text-gray-500">Select a transcript to start listening</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-gray-900 truncate">{currentTranscript.title}</h3>
            <p className="text-sm text-gray-500 truncate">{currentTranscript.source}</p>
          </div>
        </div>
          ...
          <>
            ...
            <div className="flex items-center justify-center space-x-4">
              ...
              <Button
                ...
                className="h-12 w-12 bg-gradient-to-r from-blue-500 to-green-500 hover:from-blue-600 hover:to-green-600"
              >
